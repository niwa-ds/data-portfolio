# Case03：キャンペーン反応予測と施策判断  
**（Python / ロジスティック回帰 / 統計検定）**

## 背景・想定シナリオ
直近で実施したキャンペーンの結果をもとに、  
次回キャンペーンにおいて **反応が期待できる顧客を事前に特定したい** という要望がある。

一方で、過去の施策では
- 全顧客一律配信による費用対効果の低下
- 属性による反応差が感覚的に語られている状態

といった課題が存在している。

本ケースでは、**モデルと統計的検証を用いて、  
「誰に優先的に配信すべきか」を判断すること**を目的とする。

---

## 分析目的
- キャンペーン反応（有無）に影響する要因を特定する
- 反応確率を用いて配信優先順位を設計する
- モデル結果を解釈し、施策判断に接続する

---

## 分析方針
- ブラックボックスモデルは用いず、**解釈可能性を重視**
- モデル結果だけに依存せず、統計検定による補足検証を行う
- 精度向上よりも、「なぜそう判断できるか」を重視する

---

## 使用データ
- 顧客属性データ（収入、学歴、パートナー有無など）
- 過去キャンペーンへの反応ログ（反応あり／なし）

※ 実データは含まれていません。

---

## 分析プロセス

### 1. Pythonによる前処理・特徴量設計
- 欠損値・外れ値の確認
- カテゴリ変数のエンコーディング
- モデル投入用データセットの作成

前処理はすべてPythonで行い、  
モデル・検定で共通して使用するデータを整備した。

---

### 2. ロジスティック回帰による反応予測
- 目的変数：キャンペーン反応有無
- 説明変数：顧客属性情報

ロジスティック回帰を用いて、
- 各特徴量が反応確率に与える影響
- 係数およびオッズ比

を確認した。

---

### 3. 統計検定による補足検証
モデル結果の解釈を補強するため、以下を実施。

- Welchのt検定：収入水準の差の検証
- χ²検定：カテゴリ属性と反応有無の関係確認

単一手法に依存せず、  
**複数の視点から結果の妥当性を確認**した。

---

## 結果・解釈

### 主な結果
- **収入の高さ**がキャンペーン反応に有意な影響を持つ
- パートナー有無・学歴は単独では有意でない
- ただし、他属性と組み合わせることで
  反応傾向に影響する可能性が示唆された

---

## 結論
- 一律配信ではなく、**収入を軸とした配信優先順位付け**が有効
- 単一属性ではなく、複数属性の組み合わせによる解釈が必要

---

## ネクストアクション（施策提案）
- 収入を基準に配信優先度を設定
- 以下のようなセグメント別施策を検討
  - 高収入 × パートナー有無
  - 高収入 × 学歴
- セグメントごとに訴求メッセージを出し分けることで、
  反応率向上を狙う

---

## 本ケースのポイント
- ロジスティック回帰を「予測」ではなく **判断材料** として使用
- 係数・オッズ比を用いた解釈重視の分析
- モデル結果と統計検定を組み合わせた多面的検証
- 「なぜその施策判断に至ったか」を説明可能な設計

---

## ディレクトリ構成
```text
case03/
├─ README.md
├─ notebooks/      # 前処理・モデリング・検定
├─ src/            # 前処理・分析ロジック（必要に応じて）
└─ reports/        # 図表・要約資料
